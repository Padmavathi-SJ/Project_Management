...
import { Home, CheckSquare, FileText, Calendar, LogOut } from 'lucide-react';
...
function Guide_navbar({ isOpen, toggleSidebar }) {
  ...
  return (
    <div className={`fixed top-0 pb-5 left-0 h-screen bg-white flex flex-col py-6 overflow-y-auto shadow-2xl z-50 transition-[width] duration-500 ease-in-out
         ${isOpen ? "w-64" : "w-24"}`}>
      <button
      ...
      <div className="h-32 bg-white mt-6">
       ...
       <div className="bg-white px-2">
        ...
        <Link to="schedule-review" className={`${navDiv("schedule-review")} group`}>
          <Calendar size={24} className={navIcon("schedule-review")} />
          <p className={navText("schedule-review")}>Schedule Review</p>
        </Link>
